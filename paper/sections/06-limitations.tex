...
\paragraph{No real-time OSD tail; no \texttt{NGROUP}.}
% src: docs/perf/qec-q7-fixed-bp.md "OSD-0 tail"; docs/qec/q7-07-nonconvergence-policy.md "Latency, and a caveat about the parallel timing column"; docs/perf/q7-02-ngroup-sweep.md
An OSD-0 post-processor on the flagged shots removes 76--96\,\% of the logical error rate at the
operating points measured, but costs about \SI{1.5}{\milli\second} per flagged shot on a processor,
three orders of magnitude over the per-round budget; it is a host-side escalation, not a real-time
fallback, until the posteriors are exported or OSD-0 gets a fixed-latency engine. A grouped partial-unroll knob
