...
% src: docs/qec/q7-07-nonconvergence-policy.md "The verdict under the pre-registered rule"
...
and 16 solves per shot). Every candidate beats the baseline at every point,
by a wide margin under a paired McNemar test (every $\chi^2 > 660$): OSD-0
turns the conditional error rate of 71\,\% / 83\,\% / 87\,\% at
$p = 0.003$ / 0.005 / 0.007 into 3.0\,\% / 9.5\,\% / 19\,\%, and the higher
orders give identical counts, so OSD-0 is the whole ladder. Propagated as
$\Delta\mathrm{LER} =
r(p)\,[P(\mathrm{err}\mid v{=}0) - P(\mathrm{err}\mid v{=}0,\mathrm{fallback})]$,
running OSD-0 on flagged shots lowers LER by 96\,\% / 88\,\% / 76\,\%
($8.3\times10^{-4} \to 3.5\times10^{-5}$ at $p = 0.003$). (An earlier
version of this measurement, with OSD's columns ordered by $|\mathrm{LLR}|$
instead of posterior LLR, found every candidate worse than baseline; the
numbers here are from the corrected decoder.)
% src: docs/qec/q7-07-nonconvergence-policy.md "Latency, and a caveat about the parallel timing column"
The candidates fail on latency: single-threaded on an EPYC 8124P, a
processor-side tail costs \SIrange{1475}{1646}{\micro\second} per shot it
...
Under the pre-registered rule this is the \emph{rejected-on-latency} branch.
The real-time policy is therefore \emph{do-nothing-but-flag}: emit the
best-kept decision, export \texttt{valid\_flag}, no RTL change. Off the
real-time path the flag is worth far more than telemetry: escalating the
0.12--3.3\,\% of flagged shots to a host running OSD-0 recovers most of the
LER. Bringing that tail within the real-time budget---by exporting the
posteriors to skip the re-decode, or a fixed-latency OSD-0 engine---is
outside this work.
