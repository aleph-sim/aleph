...
\begin{enumerate}
  ...
  \item The negative results that shaped the design and that a tape-out would have to face: an OSD
        tail that recovers most of the logical error rate but not within the real-time budget, no
        feasible \texttt{NGROUP} setting, a streaming core that does not fit, and a latch-clock hold
        failure that repair cannot buffer away (\S\ref{sec:limitations}).
\end{enumerate}
