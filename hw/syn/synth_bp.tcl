...
#   vivado -mode batch -source synth_bp.tcl -tclargs <part> <xdc> <outdir> <srcdir> [top] [rtl.sv]
#   M3 (sequential):  ... xck26-sfvc784-2LV-c kv260.xdc reports/kv260 .
#   M4 (unrolled):    ... xck26-sfvc784-2LV-c kv260.xdc reports/kv260_unroll . bp_relay_unrolled bp_relay_unrolled.sv
# `top`/`rtl.sv` default to the M3 sequential decoder for backward compatibility.

if {$argc < 4} {
  puts "ERROR: usage: synth_bp.tcl <part> <xdc> <outdir> <srcdir> \[top\] \[rtl.sv\]"
  exit 2
}
...
set top    [expr {$argc > 4 ? [lindex $argv 4] : "bp_relay_decoder"}]
set rtl    [expr {$argc > 5 ? [lindex $argv 5] : "bp_relay_decoder.sv"}]
file mkdir $outdir

# RTL + the generated graph header (resolved via the include path on synth_design).
read_verilog -sv [file join $src $rtl]
read_xdc $xdc

synth_design -top $top -part $part -mode out_of_context -include_dirs $src
