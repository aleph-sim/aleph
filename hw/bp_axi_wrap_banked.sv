// Q7-02 M7 — WIDE PS<->PL AXI4-Lite wrapper for the K-BANKED relay-BP decoder (`bp_relay_banked`).
//
// Same register map / word layout as `bp_axi_wrap_wide` (it is that file with the core instance and
// IDCODE swapped): graph-generic in BP_C/BP_N via the included header. Wraps the M7 banked core —
// beta-split check-major LUTRAM banks, W=BP_BANK_W checks + V=BP_BANK_V vars per cycle (baked in the
// header), ~2460 cycles/decode at 12/36 vs bram_dp's ~672k. IDCODE bumped so the runner can tell the
// two overlays apart.
//
...
module bp_axi_wrap_banked #(
  ...
  localparam logic [31:0] IDCODE = 32'h4250_0003;
