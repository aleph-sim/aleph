...
///
/// Used by the controlled SIMD kernels (P1-03): the outer loop counts `k`
...
pub(crate) fn expand_with_fixed(k: usize, fixed: &[(u32, bool)]) -> usize {
    ...
    let mut pos: u32 = 0;
    // Stop when both streams are exhausted: no more free k-bits AND no
    // more fixed entries to place.
