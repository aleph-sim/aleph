...
fn main() {
    ...
    // If `oracle/fixtures/` is missing (sparse checkout, Docker layer
    // shipping only source), emit zero generated tests rather than
    ...
    let entries: Vec<PathBuf> = if fixtures_dir.is_dir() {
        ...
        println!(
            "cargo:warning=oracle/fixtures/ not present; emitting zero generated oracle tests"
        );
        Vec::new()
    };

    // Symmetry check: every `.qasm` under `oracle/circuits/` must have
    ...
    //
    // Skipped if `oracle/circuits/` is missing entirely (sparse
    // checkout, Docker layer that ships only fixtures); the build
    // still succeeds in those environments.  Since the crate is
    // `publish = false`, registry-consumer visibility of the
    // cargo:warning is not a concern.
