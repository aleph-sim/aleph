//! Q6-32 Milestone C (the next layer of Shor's ladder) — emit n-bit modular-multiplier `y := (a·x) mod N`
//! trials (out-of-place, |x⟩|0⟩ → |x⟩|(a·x) mod N⟩) whose T-gate magic-state measurements are each resolved
//! by the on-board decoder. This is the operation Shor's modular exponentiation is built from (as its
//! controlled version): a product is n controlled-modular-additions of the classical constants a·2^i mod N.
//! Its T-count climbs another order over the modular adder — 70n² T-gates (280/630 for n=2/3), n VBE
//! modular adders deep.
//!
//! Each bit x[i] of the multiplier controls a modular add of the classical constant c_i = a·2^i mod N into
//! the accumulator y. The control is a CNOT-load of c_i into the addend register iff x[i] (Clifford), then
//! an UNCONDITIONAL VBE modular adder (70n T = 10n Toffolis) — which is the identity when the addend is 0
//! (x[i]=0). So the whole multiply is n VBE modular adders = 70n² Toffoli-borne T-gate decodes on the real
//! Arty (Q6-20 bitstream unchanged); X/CNOT (constant load, controlled load) are Clifford (no decode).
//!
//! Per trial we emit a residue input x (0 ≤ x < N) and 70n² independent memory-Z blocks (one per T-gate)
//! with their true logical flip e; the board decodes each on the Arty, drives the conditional S in a
//! (4n+3)-qubit state-vector VBE multiplier (`uf_qubit_mulmod.py`), and verifies y == (a·x) mod N — ON vs OFF.
//!
//! Usage:
//!   cargo run --release -p aleph-qec --example qec_q6_mulmod -- [n] [N] [a] [d] [W] [C] [rounds] [trials] [seed] [p]
...
//!
//! Output (stdout):
//!   # comment metadata (n/N/a/nbits=4n+3/gates=70n^2/d/W/C/dpr/slices/...)
//!   P p=<p> trials=<N>
...
fn main() {
    ...
    let gates = 70 * n * n; // n VBE modular adders x 70n T
    ...
    assert_eq!(
        dem.observables, 1,
        "modular-multiplier T-gate decode assumes a single logical observable per block"
    );
    ...
    println!("# Q6-32 modular multiplier (VBE y:=(a*x) mod N) trials — GENERATED, do not edit.");
    println!(
    ...
    println!(
        "# regenerate: cargo run --release -p aleph-qec --example qec_q6_mulmod -- {n} {modulus} {a_const} {d} {w} {c} {rounds} {trials} {seed} {p}"
    );
