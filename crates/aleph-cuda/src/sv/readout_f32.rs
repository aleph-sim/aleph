//! GPU-resident readout for the **FP32** state-vector backend (P5.11-04).
//!
//! The FP32 mirror of [`crate::sv::readout::GpuReadout`]: measurement, sampling,
//! expectation, and marginal probabilities reduce the device-resident FP32 state
//! **on the GPU** and copy back only the small result (a scalar, a `2^k` marginal,
//! or `shots` indices), never the full `2^n` amplitudes. The reduction kernels
//! (`readout_f32.cu`) read `cplxf` amplitudes but accumulate in `double`, so the
//! algorithms are byte-identical to the FP64 path — only the amplitude scalar and
//! the norm-drift budget differ (FP32 carries ~1e-5 error, not ~1e-10).
//!
//! Kept a separate struct (rather than generalising `GpuReadout`) so the
//! production FP64 + cuStateVec readout is untouched — the same f32-mirror idiom
//! the crate already uses for `kernels_f32.cu` / `paged_f32.rs`.
...
use crate::sv::fp32::CudaSvStateF32;
use crate::{CudaContext, DeviceBuffer, Error};

/// Threads per block; must match `BLOCK` in `readout_f32.cu`.
const BLOCK: u32 = 256;

/// See `aleph_sv::measure::DEGENERATE_BRANCH_THRESHOLD`. Same value as the FP64
/// path — an exactly-zero theoretical branch is what this guards (FP32 noise on a
/// real branch never reaches it).
const DEGENERATE_BRANCH_THRESHOLD: f64 = 1e-300;

/// Norm²-drift budget for the FP32 state. The FP64 readout uses
/// `√n · AMPLITUDE_TOL` (`AMPLITUDE_TOL = 1e-10`), far tighter than FP32's ~1e-5
/// accuracy — that budget would reject every legitimate FP32 state. A fixed `1e-3`
/// relative budget admits real FP32 drift while still catching gross corruption
/// (a zeroed half-state ⇒ norm²≈0.5, a NaN ⇒ caught by the finiteness check).
const FP32_NORM_DRIFT: f64 = 1e-3;

/// CUDA C++ source for the FP32 readout kernels (compiled once via NVRTC).
const READOUT_F32_SRC: &str = include_str!("readout_f32.cu");

/// Compiled FP32 readout kernels plus a small reusable accumulator.
pub(crate) struct GpuReadoutF32 {
    ctx: CudaContext,
...
impl GpuReadoutF32 {
    /// Compile the FP32 readout kernels on `ctx`'s device.
    pub(crate) fn new(ctx: &CudaContext) -> Result<Self, Error> {
        let ptx = compile_ptx(READOUT_F32_SRC).map_err(|e| Error::Compile(e.to_string()))?;
    ...
    /// Second pass: collapse the per-block pairs to the scalar `(out[0], out[1])`.
    ...
    fn reduce_branch(&mut self, state: &CudaSvStateF32, qbit: u64) -> Result<(f64, f64), Error> {
        ...
        let cfg = launch_cfg(n);
        {
            ...
            // SAFETY: kernel (const cplxf*, double* partials, u64 N, u64 qbit); args
    ...
    /// Validate finiteness + normalization (FP32 drift budget).
    fn check_norm(&mut self, state: &CudaSvStateF32) -> Result<f64, BackendError> {
        ...
        if (total - 1.0).abs() > FP32_NORM_DRIFT {
            return Err(BackendError::InvalidState {
                reason: "state norm² deviates from 1 beyond FP32 drift budget",
            });
        }
        Ok(total)
    }

    /// Collapse onto the measured branch of `qubit`, returning the outcome.
    ...
    fn collapse(
        ...
        // SAFETY: kernel (cplxf* amps, u64 N, u64 qbit, u32 outcome, f64 scale);
    ...
    pub(crate) fn expectation_value(
        ...
        let cfg = launch_cfg(n);
        {
            let amps = state.amps.slice();
            let partials = self.partials.as_mut().expect("partials set").slice_mut();
            // SAFETY: kernel (const cplxf*, double* partials, u64 N, u64 flip, u64 sign).
            unsafe {
                stream
    ...
    pub(crate) fn probabilities(
        ...
        let kk = k as u32;
        {
            ...
            // SAFETY: kernel (const cplxf*, double* out, u64 N, const u32* pos, u32 k);
    ...
    pub(crate) fn sample(
        ...
        let mut b = DeviceBuffer::<f64>::zeros(&self.ctx, n as usize).map_err(to_be)?;
        {
            ...
            // SAFETY: kernel (const cplxf*, double* probs, u64 N); probs length 2^n.
            unsafe {
                stream
        ...
        let mut d: u64 = 1;
        while d < n {
            // SAFETY: kernel (const double* in, double* out, u64 N, u64 d); distinct
            // buffers of length 2^n, grid covers N.
            if src_is_a {
...
/// Map a CUDA-layer error to a backend error (same shape the backends use).
