...
The project has completed **Phases 0–4, 4.5, the Apple/Metal GPU track (5.5 + 5.6 + 5.7 + 5.8), Phase 5 (CUDA/cuQuantum, P5-01–08), Phase 5.9 (CUDA SV → Aer-GPU parity, P5.9-01–06), and Phase 5.10 (single-GPU reach & throughput, P5.10-01–03)**. Phase 4 shipped the v0.1 public release (CLI + Python wheels). Phase 4.5 (CPU parity) closed with **every competitive-matrix cell ≤ 1.2× its reference** — aleph beats Aer multi-thread statevector on 3/4 Tier-1 workloads at n=25, beats Aer MPS 4.4–14× on all five MPS cells, and beats Stim on the surface-code cycle at every distance (`docs/perf/parity.md`). Phase 5.5 stood up the **Metal GPU state-vector backend** (`MetalSvBackend`, FP32) — **4.7–6.1× the same-Mac CPU SV at n=28** (FP32-to-FP32; `docs/perf/metal.md`), oracle-verified to 1e-5. Phase 5.6 productionized it: batched command-buffer dispatch, parallel host readout, an MPS truncation/unitarity-guard pass, a full-scale honest exit bench, and GPU auto-select routing. Phase 5.7 made the **Metal MPS backend GPU-resident and feature-complete** — GPU-resident one-sided Jacobi SVD, layer-batched dispatch, full readout (measure/sample/probabilities/expectation, no `2^n`), a SWAP router for non-NN gates, and canonical-form real truncation (`docs/perf/phase5.7.md`); it is oracle-correct vs CPU MPS + exact FP64 SV but at the time was 12–190× slower than the f64 CPU MPS. Phase 5.8 made the Metal MPS backend **fully GPU-resident** — device-buffer pool (zero steady-state allocation), the entire per-gate split (contract→apply→pack→Jacobi→σ-sort→truncate→assemble) on one command buffer with U/V/σ never read back, GPU Householder QR centre moves fused onto the gate command buffer, and lazy-permutation SWAP routing (`docs/perf/phase5.8.md`). It eliminated every host round-trip the Phase 5.7 audit named, but its exit metric (GPU MPS ≥ CPU MPS on ≥1 regime) is **still unmet**: ~16× slower than the f64 CPU MPS at n=16 χ=256 and the gap *grows* with n (~22× at n=20). The residual wall is the **single-threadgroup per-block factorisation** (Jacobi/QR), which caps parallelism far below an O(χ³) block; a multi-threadgroup factorisation is the precondition for any crossover. Net: the Apple/Metal MPS track is **architecturally complete but performance-uncompetitive with the CPU MPS** at the measured sizes. **Phase 5 (CUDA/cuQuantum) is complete and its exit metric met.** P5-01–05 built the foundation (cudarc plumbing, FP64 NVRTC state-vector kernels, the cuStateVec backend, a retaining stream-ordered memory pool, GPU-resident readout). P5-06 added **custom diagonal-gate kernels** that beat cuStateVec 1.7–2.4× in the L2-resident regime (`docs/perf/p5-06-custom-kernels.md`). P5-07 added a **device-resident GPU stabilizer tableau** that beats Stim 3–12× on Clifford traffic at n=1000–65000 (`docs/perf/p5-07-gpu-stabilizer.md`). P5-08 published the benchmark report: aleph's FP64 GPU state vector is **within 1.22× of cuStateVec on every Tier-1 + Tier-2 workload** at n=28 (median ~1.01×; exit gate ≤1.5× cleared) — but Qiskit **Aer-GPU was then 2–5× faster than both aleph and cuStateVec**, via gate fusion + batched dispatch above the kernel (`docs/perf/p5-08-gpu-report.md`). **Phase 5.9 (CUDA SV throughput → Aer-GPU parity) closed that gap and its exit metric is met** (`docs/perf/p5.9-gpu-fusion.md`): at n=28, **every Tier-1 + Tier-2 cell is within 1.5× of Aer-GPU** (worst VQE 1.25×; worst Tier-1 GHZ 1.19×), aleph now **beats Aer-GPU outright on QFT/QPE/QAOA** (0.82–0.90×) and **beats cuStateVec on every cell** (up to 4.4×). The levers — all bandwidth-pass reduction, no new precision — were: feed the IR fusion passes into `CudaSvBackend` (P5.9-01), `UnitaryKq` k≤5 apply + `FuseKq` (sweet spot width 3; k=4,5 regress on the generic matvec) (P5.9-02), disjoint-1q-layer batched dispatch via `run_layered`/`apply_1q_multi` (width 3) (P5.9-03), a CNOT permutation kernel `apply_cnot` (P5.9-04), the re-bench/verdict (P5.9-05), and a GPU diagonal phase-polynomial kernel `apply_phase_poly` that halved QFT/QPE (P5.9-06). **Phase 5.10 (single-GPU SV: reach & throughput) is complete.** P5.10-01 added a warp-cooperative **register-tiled fused-block kernel** (`apply_kq_tiled`, one amplitude per warp lane + shuffle-reduction matvec): it beats the generic `apply_kq` at every fusion width (1.07–1.18×) and is now the production default at k≤3 (~1.07× on dense cells), but k=4,5 fusion **still loses to k=3** — the wall past k=3 is the O(4^k) matvec compute, not the register spill, so `MAX_FUSE_QUBITS` stays **3** (`docs/perf/p5.10-01-tiled-fused-block.md`). P5.10-02 added out-of-core **host-memory paging** (`run_paged`): the `2^n` state lives in pinned host memory and streams device-sized tiles through the GPU, reaching **n=31 on the 20 GiB card** (norm-exact) at ~18.5× the in-core cost — PCIe-bound, a *reach* lever not a speed lever (n=32 needs 64 GiB host RAM, past the box's 62 GiB) (`docs/perf/p5.10-02-host-paging.md`). P5.10-03 added an **FP32 backend** (`CudaSvBackendF32`): a float amplitude buffer for **2.03× throughput at 0.50× memory** and **n=31 in-core** (+1 qubit past FP64), oracle-equal to FP64 within 6e-8 across Tier-1+2 (`docs/perf/p5.10-03-fp32.md`). **Phase 6 (distributed multi-GPU) stays deferred** until multi-GPU hardware exists — its scaling-efficiency exit is unmeasurable on one card. Current release: **v0.2** (PyPI: `aleph-sim`). The CUDA GPU box is `openwebgui.splynx.com` (RTX 4000 SFF Ada sm_89; CUDA 13.0 + cuQuantum; stim + qiskit-aer-gpu venvs at `/root/stimvenv` and `/root/aervenv`). See `ROADMAP.md` § 5/§ 7 for phase definitions and exit metrics.
