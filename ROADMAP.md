...
- Phase 5.9 (CUDA SV throughput → Aer-GPU parity), exit = aleph FP64 GPU state vector **within 1.5× of Qiskit Aer-GPU (FP64)** on the Tier-1 + Tier-2 suite at n=28, no Tier-1 cell worse than 2× (currently 2–5× behind; see `docs/perf/p5-08-gpu-report.md`). The levers — feed the existing `Fuse2q`/diagonal-run IR passes into `CudaSvBackend`, 4–5q fused dense blocks, disjoint-layer single-sweep dispatch — are bandwidth-pass reduction, not new precision or kernels.
