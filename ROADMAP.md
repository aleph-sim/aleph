...
- Phase 5.9 (CUDA SV throughput → Aer-GPU parity), exit = aleph FP64 GPU state vector **within 1.5× of Qiskit Aer-GPU (FP64)** on the Tier-1 + Tier-2 suite at n=28, no Tier-1 cell worse than 2×. ⚠️ **PARTIALLY met** (P5.9-05 verdict, `docs/perf/p5.9-gpu-fusion.md`): from 2–5× behind on every cell (P5-08) to **5/7 cells within 1.5× of Aer-GPU** at n=28 — dense-2q (random/VQE/QAOA) + Grover at parity (1.03–1.25×), where aleph now also beats cuStateVec 1.7–4.4×. Shipped levers: IR fusion into the GPU path (P5.9-01), `UnitaryKq` k≤5 apply + `FuseKq` width-3 (P5.9-02), disjoint-1q-layer batched dispatch (P5.9-03), CNOT permutation kernel (P5.9-04). Only **QFT (1.73×) and QPE (1.75×)** remain over — the controlled-phase ladder doesn't fuse into dense blocks; closing it needs the one unbuilt lever, a GPU diagonal phase-polynomial kernel (`FuseDiagonalRuns` → one-sweep `exp(i·φ(x))`).
