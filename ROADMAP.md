...
- Phase 5.10 (single-GPU SV: reach & throughput) — **active next track**. The levers that still have a measurable exit on the single RTX 4000 Ada box: a **register-tiled fused-block kernel** to unlock k=4,5 fusion (P5.10-01; the generic `apply_kq` regresses past k=3 on register spill), **host-memory paging** for n>30 out-of-core on one GPU (P5.10-02), and an **FP32 / mixed-precision SV** for ~2× memory+throughput and +1 qubit (P5.10-03). See `BACKLOG.md`.
- Phase 6: Distributed run on 4+ nodes with reasonable scaling efficiency. **Deferred until multi-GPU hardware is available** — its exit metric (scaling efficiency across ranks) is unmeasurable on a single card, so building it now would ship an unverifiable perf claim. The correctness substrate (rank-partitioned state + halo exchange) is testable at small scale, but the perf exit needs a real multi-GPU node (a few cloud hours suffice). Until then, single-GPU reach lives in Phase 5.10.
